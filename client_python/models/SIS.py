...
class SISModel(graph_model.GraphModel):
    ...
    def handle_property_change(self, prop, value):
        if (prop == "size"):
            self.clear()
            self.SISmod(value, density=self.density, startfrac=self.startfrac, infrate=self.infrate, recrate=self.recrate, gtype=self.gtype)
        elif prop == "density":
            self.clear()
            self.SISmod(self.size, density=value, startfrac=self.startfrac, infrate=self.infrate, recrate=self.recrate, gtype=self.gtype)
        elif prop == "startfrac":
            self.clear()
            self.SISmod(self.size, self.density, startfrac=value, infrate=self.infrate, recrate=self.recrate, gtype=self.gtype)
        elif prop == "infrate":
            self.clear()
            self.SISmod(self.size, self.density, self.startfrac, infrate=value, recrate=self.recrate, gtype=self.gtype)
        elif prop == "recrate":
            self.clear()
            self.SISmod(self.size, self.density, self.startfrac, self.infrate, recrate=value, gtype=self.gtype)
        elif prop == "gtype":
            self.clear()
            self.SISmod(self.size, self.density, self.startfrac, self.infrate, self.recrate, gtype = value)
        elif prop == "speed":
            self.set_speed(value)
        elif prop == "steptype":
            self.steptype = value

    def SISmod(self, n, density, startfrac, infrate, recrate, gtype):
    ...
    def create(self):
        ...
        self.infect_count_chart = self.add_chart('line',  ### NEED NEW PLOT FUNCTION
        #self.infect_count_chart = self.add_chart('ts',
                title='Infected nodes count',
                xLabel='t',
                size=8)
        self.active_links_chart = self.add_chart('line',
        #self.active_links_chart = self.add_chart('ts',
                title='Active links count',
                xLabel='t',
                size=8)

        self.SISmod(size, density, startfrac, infrate, recrate, gtype)
    ...
    def reset(self):
        self.clear()
        self.SISmod(self.size, self.density, self.startfrac, self.infrate, self.recrate, self.gtype)
    ...
    def sync_step(self):
        ...
        for n in recover_set:
            self.update_node_props(n, color='green')
            self.update_node_props(n, state='susceptible')
            self.susceptible_set.add(n)
    ...
    def stop_condition(self):
        return self.infect_count == 0 and self.time > 0
